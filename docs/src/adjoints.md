...
!!! note "Prefer to use ChainRules to define custom adjoints"
    ...
    It is prefered to define the custom sensitivities using ChainRulesCore as they will work for many AD systems, not just Zygote.
    ...
    It allows for specific definations adjoints that are only defined for Zgyote (which might work differently to more generic definations defined for all AD)
